...
proto.param = function param(name, fn) {
  ...
  // apply param functions                                                               this._params是数组， this.params 是对象
  ...
  for (var i = 0; i < len; ++i) {                                                       // 遍历一下 _params数组，里面放的都是函数，  挨个执行一下，如果返回true  那么 fn = 就为最后返回true 的那个
  ...
  (this.params[name] = this.params[name] || []).push(fn);                              // 大有一番文章。。。。。。。。。。到底是什么i情况？ params ={user:[fb1,fb2], foo:[fb1,fb2]}
  return this;
};
...
proto.handle = function handle(req, res, out) {                                //真正处理（req ，res）的地方在这里   每次request  都会调用这个函数一次。   out是输出，http的最后执行的一步，要么返回404 要么打印错误。
  ...
  var protohost = getProtohost(req.url) || ''                                  //req.url   只是  后面的 /user/abc      没有前面的  域名：端口    原主机
  var removed = '';
  var slashAdded = false;                                                      // 增加斜线
  var paramcalled = {};                                                         // 需要的参数
  ...
  var parentParams = req.params;                                           // 这个params  不是req的自带的参数，应该是express给他添加的
  var parentUrl = req.baseUrl || '';                                       // baseUrl也米有呀！
  ...
  function next(err) {
    ...
    var layerPath = layer.path;                           // 构建路由的时候， get （path ,function） 这个path
    ...
    self.process_params(layer, paramcalled, req, res, function (err) {                                //paramcalled是传入的空对象，
      ...
      trim_prefix(layer, layerError, layerPath, path);                                 // 如果不是路由中间件， 也就是普通的中间件 。 执行这个函数。   修剪前缀？？  这个函数里面 也有handle_request函数
    });
  }

  function trim_prefix(layer, layerError, layerPath, path) {
    if (layerPath.length !== 0) {
      // Validate path breaks on a path separator                                  验证路径分隔符上的路径中断
...
proto.process_params = function process_params(layer, called, req, res, done) {
  var params = this.params;                                                   //router的params  是由app.param(name,fb)  添加到里面的params ={user:[fb1,fb2], foo:[fb1,fb2]}       而layer 的param是由 正则表达式匹配的时候产生的params{user=123, foo=abc}
  ...
  var i = 0;
  var name;
  var paramIndex = 0;                                                // param是 路由的参数，   layer也有多个参数，  需要匹配的话， 那就是多对多的关系
  var key;
  var paramVal;
  var paramCallbacks;
  var paramCalled;
  ...
  function param(err) {
    ...
    paramIndex = 0;                                                    // router 的param 每次都是从头开始  去遍历layer的keys      （一次的请求可能有多个key， router可能有多个param参数，多对多关系）
    key = keys[i++];
    name = key.name;
