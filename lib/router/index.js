...
proto.handle = function handle(req, res, out) {                                //真正处理（req ，res）的地方在这里   每次request  都会调用这个函数一次。
  var self = this;

  debug('dispatching %s %s', req.method, req.url);

  var idx = 0;
  var protohost = getProtohost(req.url) || ''                                  //原主机
  var removed = '';
  ...
  function next(err) {
    ...
    while (match !== true && idx < stack.length) {
      layer = stack[idx++];
      match = matchLayer(layer, path);                        // 遍历路由  匹配一下路径， 是不是所对应的连接
      route = layer.route;
    ...
    self.process_params(layer, paramcalled, req, res, function (err) {
      ...
      if (route) {
        return layer.handle_request(req, res, next);                                     // 匹配的上了，  用对应layer 的handle去处理一下，返回结果
      }
